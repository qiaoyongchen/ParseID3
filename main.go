...
// GetSize 获取标头长度
func (p *ID3v2Header) GetSize() int64 {
	return int64(p.Size[0]&0x7f)*0x200000 +
		int64(p.Size[1]&0x7f)*0x4000 +
		int64(p.Size[2]&0x7f)*0x80 +
		int64(p.Size[3]&0x7f)
}
...
// GetSize 获取帧内容大小
func (p *ID3v2Frame) GetSize() int64 {
	return int64(p.Size[0])*0x1000000 +
		int64(p.Size[1])*0x10000 +
		int64(p.Size[2])*0x100 +
		int64(p.Size[3])
}
...
// GetCover 获取封面
func (p *ID3v2) GetCover() ([]byte, []byte) {
	if _, frexist := p.Frames[COVER_PHOTO_ID]; !frexist {
		return nil, nil
	}
	var i int64
	bytes := p.Frames[COVER_PHOTO_ID].Content
...
// GetFrameContent 获取帧体内容:string(utf8)
func (p *ID3v2) GetFrameContent(fID string) ([]byte, error) {
	...
	var fr *ID3v2Frame
	var ifinframes bool
	if fr, ifinframes = p.Frames[fID]; !ifinframes {
